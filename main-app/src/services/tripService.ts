...
import type { Trip, Expense, ServiceResponse } from "../types";
...
export const tripService = {
  ...
  async getById(id: string): Promise<ServiceResponse<Trip>> {
    ...
    const [tripRes, fuelRes, tollRes, miscRes, expRes] = await Promise.all([
      query(
        supabase.from("trips").select("*").eq("id", id).eq("owner_id", uid).single()
      ),
      query(
        supabase.from("fuel_logs").select("*").eq("trip_id", id).eq("owner_id", uid).order("date", { ascending: false })
      ),
      query(
        supabase.from("toll_logs").select("*").eq("trip_id", id).eq("owner_id", uid).order("date", { ascending: false })
      ),
      query(
        supabase.from("misc_expenses").select("*").eq("trip_id", id).eq("owner_id", uid).order("date", { ascending: false })
      ),
      query(
  ...
  async create(
    data: Omit<Trip, "id" | "owner_id"> & { status?: string }
  ): Promise<ServiceResponse<Trip>> {
    return query(
      supabase
        .from("trips")
        .insert({ ...data, owner_id: getUid(), status: data.status ?? "planned" })
        .select()
        .single()
    );
  },
  ...
  async deleteExpense(id: string): Promise<ServiceResponse<null>> {
    return query(supabase.from("expenses").delete().eq("id", id));
  },
};
