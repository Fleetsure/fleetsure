...
def add_expense(trip_id: UUID, payload: ExpenseCreate, db: Session = Depends(get_db)):
    ...
    **Expense types:**
    - `fuel` — Diesel/petrol costs
    - `toll` — Highway toll charges
    - `maintenance` — Repairs, tyres, etc.
    - `driver_payment` — Driver salary/advance for this trip
    - `loading_unloading` — Labour charges at origin/destination
    - `police_challan` — Fines
