...
def add_fuel_log(payload: FuelLogCreate, db: Session = Depends(get_db)):
...
def get_fuel_logs(vehicle_id: UUID = None, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    q = db.query(FuelLog).order_by(FuelLog.date.desc())
    if vehicle_id:
...
def delete_fuel_log(log_id: UUID, db: Session = Depends(get_db)):
...
def fuel_analytics(db: Session = Depends(get_db)):
    ...
    for v in vehicles:
        ...
        # Calculate km/l for each interval
        kmpl_values = []
