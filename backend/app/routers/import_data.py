...
TRIP_FIELDS: Dict[str, List[str]] = {
    "registration_number": ["vehicle", "truck", "reg no", "registration", "vehicle no", "truck no",
                            "veh", "vehicle reg", "truck reg", "vehicle number"],
    ...
    "destination":      ["to", "destination", "to city", "delivery", "unloading", "unloading point", "destination"],
    "start_date":       ["start date", "start", "trip start", "trip start date", "date", "trip date",
                         "departure", "from date", "departure date"],
    "end_date":         ["end date", "end", "trip end", "trip end date", "arrival", "to date",
                         "completion", "arrival date"],
    "freight_amount":   ["freight", "amount", "revenue", "income", "freight amount", "billing",
                         "charge", "rate", "trip amount", "total amount", "total freight"],
    "distance_km":      ["distance", "dist", "km", "distance km", "kms", "total km", "km travelled",
                         "distance covered", "trip distance"],
    "driver_advance":   ["advance", "cash", "driver advance", "cash advance", "advance paid",
                         "cash given", "driver cash", "advance amount"],
    ...
    for i, row in enumerate(sheet.rows, start=2):
        ...
        try:
            t = Trip(
                ...
                distance_km=parse_num(_mapped(row, m, "distance_km")),
                driver_advance=parse_num(_mapped(row, m, "driver_advance")) or Decimal("0"),
