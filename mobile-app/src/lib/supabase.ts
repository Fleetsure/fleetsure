...
// Wait for firebase/auth to restore the persisted session before any
// Supabase request fires — otherwise auth.currentUser is null and the token
// exchange has nothing to exchange.
...
async function authFetch(
  ...
  try {
    ...
    // A signed-in user whose token exchange fails (network blip, edge
    // function error, JWKS fetch failure) must not fall through to an
    // unauthenticated request — that hits every table as the anonymous
    // role and RLS silently returns zero rows everywhere, with nothing in
    // the app surfacing why. Only log when a Firebase user actually exists,
    // since that's the case that indicates a real failure rather than the
